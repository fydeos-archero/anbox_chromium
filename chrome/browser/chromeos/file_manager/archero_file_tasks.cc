...
//             TASK_TYPE_ANBOX_APP, ArcActionToFileTaskActionId(action)),
...
void FindArcTasksAfterContentUrlsResolved(
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    LOG(INFO) << "==== FindArcTasksAfterContentUrlsResolved " << entry.path.Extension();
    if (0 != strcasecmp(entry.path.Extension().c_str(), ".apk")){
      result_list->push_back(FullTaskDescriptor(
        TaskDescriptor(std::string(), TASK_TYPE_UNKNOWN, std::string()),
        std::string(), extensions::api::file_manager_private::VERB_NONE, GURL(), false /* is_default */,
        // action != arc::kIntentActionView /* is_generic */,
        false,
        false /* is_file_extension_match */));
    }else{
      ...
      result_list->push_back(FullTaskDescriptor(
          TaskDescriptor(
              ...
              TASK_TYPE_ARCHERO_APP, /*ArcActionToFileTaskActionId(action)*/ "view"
            ),
          std::string("Package installer"), extensions::api::file_manager_private::VERB_NONE, GURL(), false /* is_default */,
          // action != arc::kIntentActionView /* is_generic */,
          false,
          false /* is_file_extension_match */));
    }
...
// void ExecuteArcHeroTaskAfterContentUrlsResolved(
//     Profile* profile,
//     const TaskDescriptor& task,
//     const std::vector<std::string>& mime_types,
...
//   LOG(INFO) << "==== ExecuteArcHeroTaskAfterContentUrlsResolved " << task.app_id << " " << task.action_id << " " << task.task_type;

//   storage::FileSystemContext* file_system_context =
//       util::GetFileSystemContextForExtensionId(profile, kFileManagerAppId);

//   for (size_t i = 0; i < content_urls.size(); ++i) {
//     LOG(INFO) << "==== ExecuteArcHeroTaskAfterContentUrlsResolved2 " << content_urls[i];
//     LOG(INFO) << "==== ExecuteArcHeroTaskAfterContentUrlsResolved3 " << file_system_context->CrackURL(content_urls[i]).path();

//     // base::File f(base::FilePath(content_urls[i]));
//     // arc::ArcHeroSessionManager::Get()->GetArcHeroSession()->InstallApp(content_urls[i].possibly_invalid_spec());
...
//   (void)ConstructUrlWithMimeTypeList;
//   (void)ConstructOpenUrlsRequest;
...
void FindArcHeroTasks(Profile* profile,
                  const std::vector<extensions::EntryInfo>& entries,
  ...
  for (const GURL& file_url : file_urls) {
    LOG(INFO) << "=== FindArcHeroTasks " << file_system_context->CrackURL(file_url).path();
...
void ExecuteArcHeroTask(Profile* profile,
                    const TaskDescriptor& task,
  ...
  for (size_t i = 0; i < file_system_urls.size(); ++i) {
    LOG(INFO) << "=== ExecuteArcHeroTask " << file_system_urls[i].path();

    arc::ArcHeroSessionManager::Get()->GetArcHeroSession()->InstallApp(file_system_urls[i].path().value());
  }
  ...
  file_manager::util::ConvertToContentUrls(
      file_system_urls, base::BindOnce(&ExecuteArcHeroTaskAfterContentUrlsResolved,
