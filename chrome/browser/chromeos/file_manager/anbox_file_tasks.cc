    ...
    if (0 != strcasecmp(entry.path.Extension().c_str(), ".apk")){
      ...
      result_list->push_back(FullTaskDescriptor(
          TaskDescriptor(
              ...
              TASK_TYPE_ANBOX_APP, /*ArcActionToFileTaskActionId(action)*/ "view"
            ),          
...
void ExecuteAnboxTaskAfterContentUrlsResolved(
    Profile* profile,
    const TaskDescriptor& task,
    const std::vector<std::string>& mime_types,
  ...
  LOG(INFO) << "==== ExecuteAnboxTaskAfterContentUrlsResolved " << task.app_id << " " << task.action_id << " " << task.task_type;

  storage::FileSystemContext* file_system_context =
      util::GetFileSystemContextForExtensionId(profile, kFileManagerAppId);  

  for (size_t i = 0; i < content_urls.size(); ++i) {
    LOG(INFO) << "==== ExecuteAnboxTaskAfterContentUrlsResolved2 " << content_urls[i];
    LOG(INFO) << "==== ExecuteAnboxTaskAfterContentUrlsResolved3 " << file_system_context->CrackURL(content_urls[i]).path();    
        
    // base::File f(base::FilePath(content_urls[i]));    
    // arc::AnboxSessionManager::Get()->GetAnboxSession()->InstallApp(content_urls[i].possibly_invalid_spec());
...
void FindAnboxTasks(Profile* profile,
                  const std::vector<extensions::EntryInfo>& entries,
  ...
  for (const GURL& file_url : file_urls) {    
    LOG(INFO) << "=== FindAnboxTasks " << file_system_context->CrackURL(file_url).path();
...
void ExecuteAnboxTask(Profile* profile,
                    const TaskDescriptor& task,
  ...
  for (size_t i = 0; i < file_system_urls.size(); ++i) {
    LOG(INFO) << "=== ExecuteAnboxTask " << file_system_urls[i].path();

    arc::AnboxSessionManager::Get()->GetAnboxSession()->InstallApp(file_system_urls[i].path().value());
  }
  ...
  // UI thread, where |profile| should be alive.
  file_manager::util::ConvertToContentUrls(
      file_system_urls, base::BindOnce(&ExecuteAnboxTaskAfterContentUrlsResolved,
