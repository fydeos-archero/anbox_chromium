...
class AnboxAppWindowLauncherController::AppWindowInfo {
 public:
...
AnboxAppWindowLauncherController::AnboxAppWindowLauncherController(
  ...
  // }
  LOG(INFO) << "==== AnboxAppWindowLauncherController::AnboxAppWindowLauncherController";
...
void AnboxAppWindowLauncherController::ActiveUserChanged(
...
void AnboxAppWindowLauncherController::AdditionalUserAddedToSession(
    Profile* profile) {
  DCHECK(!arc::IsArcAllowedForProfile(profile));
}

void AnboxAppWindowLauncherController::OnWindowInitialized(aura::Window* window) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnWindowInitialized";
...
void AnboxAppWindowLauncherController::OnWindowVisibilityChanged(
    aura::Window* window,
    bool visible) {
...
void AnboxAppWindowLauncherController::OnWindowDestroying(aura::Window* window) {
  auto it =
...
AnboxAppWindowLauncherController::AppWindowInfo*
AnboxAppWindowLauncherController::GetAppWindowInfoForTask(int task_id) {
...
void AnboxAppWindowLauncherController::AttachControllerToWindowsIfNeeded() {
  for (auto* window : observed_windows_)
    AttachControllerToWindowIfNeeded(window);
}

void AnboxAppWindowLauncherController::AttachControllerToWindowIfNeeded(
    aura::Window* window) {
  const int task_id = arc::GetWindowTaskId(window);
  if (task_id == arc::kNoTaskId)
    return;
...
void AnboxAppWindowLauncherController::OnAppStatesChanged(
    const std::string& app_id,
    const AnboxAppListPrefs::AppInfo& app_info) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnAppStatesChanged";

  if (!app_info.ready)
    OnAppRemoved(app_id);
}

std::vector<int> AnboxAppWindowLauncherController::GetTaskIdsForApp(
...
void AnboxAppWindowLauncherController::OnAppRemoved(
    const std::string& arc_app_id) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnAppRemoved";
...
void AnboxAppWindowLauncherController::OnTaskCreated(
    int task_id,
    const std::string& package_name,
    const std::string& activity_name,
    const std::string& intent) {
  LOG(INFO) << "==== AnboxAppWindowLauncherController::OnTaskCreated";
...
void AnboxAppWindowLauncherController::OnTaskDescriptionUpdated(
    int32_t task_id,
    const std::string& label,
    const std::vector<uint8_t>& icon_png_data) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnTaskDescriptionUpdated";
...
void AnboxAppWindowLauncherController::OnTaskDestroyed(int task_id) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnTaskDestroyed";
...
void AnboxAppWindowLauncherController::OnTaskSetActive(int32_t task_id) {
  LOG(INFO) << "=== AnboxAppWindowLauncherController::OnTaskSetActive";
...
int AnboxAppWindowLauncherController::GetActiveTaskId() const {
  LOG(INFO) << "==== AnboxAppWindowLauncherController::GetActiveTaskId " << active_task_id_;
  return active_task_id_;
}

AppWindowLauncherItemController*
AnboxAppWindowLauncherController::ControllerForWindow(aura::Window* window) {
  if (!window)
    return nullptr;
...
void AnboxAppWindowLauncherController::OnArcOptInManagementCheckStarted() {
...
void AnboxAppWindowLauncherController::OnArcSessionStopped(
    arc::ArcStopReason stop_reason) {
  opt_in_management_check_start_time_ = base::Time();
}

void AnboxAppWindowLauncherController::OnWindowActivated(
...
void AnboxAppWindowLauncherController::StartObserving(Profile* profile) {
  aura::Env::GetInstance()->AddObserver(this);
  auto prefs = AnboxAppListPrefs::Get(profile);
  DCHECK(prefs);
  prefs->AddObserver(this);
}

void AnboxAppWindowLauncherController::StopObserving(Profile* profile) {
  for (auto* window : observed_windows_)
    window->RemoveObserver(this);
  auto prefs = AnboxAppListPrefs::Get(profile);
...
AnboxAppWindowLauncherController::AttachControllerToTask(
    int task_id,
...
void AnboxAppWindowLauncherController::HandlePlayStoreLaunch(
    AppWindowInfo* app_window_info) {
  arc::Intent intent;
