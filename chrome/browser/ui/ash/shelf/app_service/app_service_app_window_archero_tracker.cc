...
AppServiceAppWindowArcHeroTracker::AppServiceAppWindowArcHeroTracker(
    AppServiceAppWindowShelfController* app_service_controller):observed_profile_(app_service_controller->owner()->profile()),
    app_service_controller_(app_service_controller) {

  auto const prefs = ArcHeroAppListPrefs::Get(observed_profile_);
  DCHECK(prefs);
...
void AppServiceAppWindowArcHeroTracker::OnTaskCreated(int32_t task_id,
  ...
  const std::string arc_app_id =
      ArcHeroAppListPrefs::GetAppId(package_name, activity_name);
...
int AppServiceAppWindowArcHeroTracker::GetTaskIdSharingLogicalWindow(int task_id) {
...
void AppServiceAppWindowArcHeroTracker::AttachControllerToWindow(
    aura::Window* window) {
  ...
  window->SetProperty(aura::client::kAppType,
                      static_cast<int>(ash::AppType::ARCHERO_APP));
...
void AppServiceAppWindowArcHeroTracker::AttachControllerToTask(int task_id) {
...
void AppServiceAppWindowArcHeroTracker::ActiveUserChanged(
...
void AppServiceAppWindowArcHeroTracker::HandleWindowVisibilityChanged(
    aura::Window* window) {
  const int task_id = arc::GetWindowTaskId(window);
...
void AppServiceAppWindowArcHeroTracker::HandleWindowDestroying(
    aura::Window* window) {
...
ash::ShelfID AppServiceAppWindowArcHeroTracker::GetShelfId(int task_id) const {
...
void AppServiceAppWindowArcHeroTracker::OnItemDelegateDiscarded(
...
void AppServiceAppWindowArcHeroTracker::OnTaskDestroyed(int32_t task_id) {
  auto it = task_id_to_arc_app_window_info_.find(task_id);
  if (it == task_id_to_arc_app_window_info_.end())
    return;
...
void AppServiceAppWindowArcHeroTracker::OnTaskSetActive(int32_t task_id) {
  if (observed_profile_ != app_service_controller_->owner()->profile()) {
