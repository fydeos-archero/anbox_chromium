...
bool InstallIconFromFileThread(const base::FilePath& icon_path,
                                const std::vector<char>& content_png) {
...
// AnboxAppListPrefs may try to touch ArcSessionManager related stuff.
...
// |info2|.
bool AreAppStatesChanged(const AnboxAppListPrefs::AppInfo& info1,
                         const AnboxAppListPrefs::AppInfo& info2) {
...
// static
AnboxAppListPrefs* AnboxAppListPrefs::Create(Profile* profile) {
  return new AnboxAppListPrefs(profile);
}

// static
void AnboxAppListPrefs::RegisterProfilePrefs(
...
// static
AnboxAppListPrefs* AnboxAppListPrefs::Get(content::BrowserContext* context) {
  return AnboxAppListPrefsFactory::GetInstance()->GetForBrowserContext(context);
}

// static
std::string AnboxAppListPrefs::GetAppId(const std::string& package_name,
...
// static
void AnboxAppListPrefs::UprevCurrentIconsVersionForTesting() {
  ++current_icons_version;
}

std::string AnboxAppListPrefs::GetAppIdByPackageName(
...
// Instances are owned by AnboxAppListPrefs.
// It performs decoding, resizing and encoding resized icon. Used to support
// legacy mojom for icon requests.
class AnboxAppListPrefs::ResizeRequest : public ImageDecoder::ImageRequest {
 public:
  ResizeRequest(const base::WeakPtr<AnboxAppListPrefs>& host,
  ...
  void OnImageDecoded(const SkBitmap& bitmap) override {
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&AnboxAppListPrefs::OnIconResized, host_, app_id_,
                       descriptor_));
 ...
 private:
  ...
  base::WeakPtr<AnboxAppListPrefs> host_;
...
AnboxAppListPrefs::AnboxAppListPrefs(Profile* profile)
  ...
  //     profile, base::BindOnce(&AnboxAppListPrefs::OnDefaultAppsReady,
...
void AnboxAppListPrefs::StartPrefs() {
  LOG(INFO) << "==== AnboxAppListPrefs::StartPrefs";

  // Don't tie AnboxAppListPrefs created with sync test profile in sync
  // integration test to ArcSessionManager.
  // if (!AnboxAppListPrefsFactory::IsFactorySetForSyncTest()) {
  //   arc::ArcSessionManager* arc_session_manager = arc::ArcSessionManager::Get();
...
base::FilePath AnboxAppListPrefs::GetAppPath(const std::string& app_id) const {
  return base_path_.AppendASCII(app_id);
}

base::FilePath AnboxAppListPrefs::MaybeGetIconPathForDefaultApp(
    const std::string& app_id,
...
base::FilePath AnboxAppListPrefs::GetIconPath(
    const std::string& app_id,
    const ArcAppIconDescriptor& descriptor) {
...
bool AnboxAppListPrefs::IsIconRequestRecorded(
    const std::string& app_id,
    const ArcAppIconDescriptor& descriptor) const {
...
void AnboxAppListPrefs::MaybeRemoveIconRequestRecord(const std::string& app_id) {
  request_icon_recorded_.erase(app_id);
}

void AnboxAppListPrefs::ClearIconRequestRecord() {
  request_icon_recorded_.clear();
}

void AnboxAppListPrefs::RequestIcon(const std::string& app_id,
                                  const ArcAppIconDescriptor& descriptor) {
  DCHECK_NE(app_id, arc::kPlayStoreAppId);
  LOG(INFO) << "==== AnboxAppListPrefs::RequestIcon " << app_id;
...
void AnboxAppListPrefs::SendIconRequest(const std::string& app_id,
                                      const AppInfo& app_info,
                                      const ArcAppIconDescriptor& descriptor) {
  base::OnceCallback<void(const std::vector<uint8_t>& /* icon_png_data */)>
      callback =
          base::BindOnce(&AnboxAppListPrefs::OnIcon,
...
void AnboxAppListPrefs::MaybeRequestIcon(const std::string& app_id,
                                       const ArcAppIconDescriptor& descriptor) {
  if (!IsIconRequestRecorded(app_id, descriptor))
    RequestIcon(app_id, descriptor);
}

void AnboxAppListPrefs::SetNotificationsEnabled(const std::string& app_id,
                                              bool enabled) {
  if (!IsRegistered(app_id)) {
...
void AnboxAppListPrefs::AddObserver(Observer* observer) {
  LOG(INFO) << "==== AnboxAppListPrefs::AddObserver";
  observer_list_.AddObserver(observer);
}

void AnboxAppListPrefs::RemoveObserver(Observer* observer) {
  LOG(INFO) << "==== AnboxAppListPrefs::RemoveObserver";
  observer_list_.RemoveObserver(observer);
}

bool AnboxAppListPrefs::HasObserver(Observer* observer) {
  return observer_list_.HasObserver(observer);
}

base::RepeatingCallback<std::string(const std::string&)>
AnboxAppListPrefs::GetAppIdByPackageNameCallback() {
  return base::BindRepeating(
      [](base::WeakPtr<AnboxAppListPrefs> self, const std::string& package_name) {
        if (!self)
          return std::string();
...
std::unique_ptr<AnboxAppListPrefs::PackageInfo> AnboxAppListPrefs::GetPackage(
    const std::string& package_name) const {
...
std::vector<std::string> AnboxAppListPrefs::GetAppIds() const {
  LOG(INFO) << "===== AnboxAppListPrefs::GetAppIds";
...
std::vector<std::string> AnboxAppListPrefs::GetAppIdsNoArcEnabledCheck() const {
  std::vector<std::string> ids;
  const base::DictionaryValue* apps =
      prefs_->GetDictionary(arc::prefs::kArcApps);
  DCHECK(apps);
...
std::unique_ptr<AnboxAppListPrefs::AppInfo> AnboxAppListPrefs::GetApp(
    const std::string& app_id) const {
  LOG(INFO) << "======  AnboxAppListPrefs::GetApp " << app_id;
...
bool AnboxAppListPrefs::IsRegistered(const std::string& app_id) const {
  LOG(INFO) << "===== AnboxAppListPrefs::IsRegistered " << app_id;
...
bool AnboxAppListPrefs::IsOem(const std::string& app_id) const {
  return false;
  // LOG(INFO) << "===== AnboxAppListPrefs::IsOem " << app_id;

  // const ArcDefaultAppList::AppInfo* app_info = default_apps_->GetApp(app_id);
  // return app_info && app_info->oem;
}

bool AnboxAppListPrefs::IsShortcut(const std::string& app_id) const {
  std::unique_ptr<AnboxAppListPrefs::AppInfo> app_info = GetApp(app_id);
...
void AnboxAppListPrefs::SetLastLaunchTime(const std::string& app_id) {
  ...
  LOG(INFO) << "===== AnboxAppListPrefs::SetLastLaunchTime-1";
  ...
  LOG(INFO) << "===== AnboxAppListPrefs::SetLastLaunchTime-2";  
...
void AnboxAppListPrefs::NotifyRegisteredApps() {
  if (apps_restored_)
    return;
...
void AnboxAppListPrefs::RemoveAllAppsAndPackages() {
  std::vector<std::string> app_ids = GetAppIdsNoArcEnabledCheck();
  for (const auto& app_id : app_ids) {
...
// void AnboxAppListPrefs::OnArcPlayStoreEnabledChanged(bool enabled) {
//   LOG(INFO) << "==== AnboxAppListPrefs::OnArcPlayStoreEnabledChanged";
...
void AnboxAppListPrefs::SetDefaultAppsFilterLevel() {
  LOG(INFO) << "==== AnboxAppListPrefs::SetDefaultAppsFilterLevel";
...
void AnboxAppListPrefs::OnDefaultAppsReady() {
  VLOG(1) << "Default apps ready";
  LOG(INFO) << "==== AnboxAppListPrefs::OnDefaultAppsReady";
...
void AnboxAppListPrefs::RegisterDefaultApps() {
  // LOG(INFO) << "==== AnboxAppListPrefs::RegisterDefaultApps";
...
base::Value* AnboxAppListPrefs::GetPackagePrefs(const std::string& package_name,
                                              const std::string& key) {
...
void AnboxAppListPrefs::SetPackagePrefs(const std::string& package_name,
                                      const std::string& key,
                                      base::Value value) {
  if (!GetPackage(package_name)) {
    LOG(ERROR) << package_name << " can not be found.";
...
void AnboxAppListPrefs::SetDefaultAppsReadyCallback(base::OnceClosure callback) {
...
void AnboxAppListPrefs::SimulateDefaultAppAvailabilityTimeoutForTesting() {
...
// void AnboxAppListPrefs::OnConnectionReady() {
//   LOG(INFO) << "==== AnboxAppListPrefs::OnConnectionReady";
...
// void AnboxAppListPrefs::OnConnectionClosed() {
//   LOG(INFO) << "==== AnboxAppListPrefs::OnConnectionClosed";
...
void AnboxAppListPrefs::HandleTaskCreated(const base::Optional<std::string>& name,
  ...
  if (IsRegistered(app_id)) {
    LOG(INFO) << "===== AnboxAppListPrefs::HandleTaskCreated-1";
    SetLastLaunchTime(app_id);
  } else {
    LOG(INFO) << "===== AnboxAppListPrefs::HandleTaskCreated-2";
...
void AnboxAppListPrefs::AddAppAndShortcut(const std::string& name,
                                        const std::string& package_name,
                                        const std::string& activity,
  ...
  LOG(INFO) << "===== AnboxAppListPrefs::AddAppAndShortcut " << name << " " << package_name
    << " " << activity << " " << intent_uri << " ";
...
void AnboxAppListPrefs::RemoveApp(const std::string& app_id) {
...
void AnboxAppListPrefs::AddOrUpdatePackagePrefs(
    const arc::mojom::ArcPackageInfo& package) {
  DCHECK(IsArcAndroidEnabledForProfile(profile_));
  const std::string& package_name = package.package_name;

  LOG(INFO) << "=== AnboxAppListPrefs::AddOrUpdatePackagePrefs";
...
void AnboxAppListPrefs::RemovePackageFromPrefs(const std::string& package_name) {
  DictionaryPrefUpdate(prefs_, arc::prefs::kArcPackages)
      .Get()
      ->RemoveKey(package_name);
}

void AnboxAppListPrefs::OnAppListRefreshed(
  ...
  if (!app_connection_holder()->IsConnected()) {
    ...
    app_list_refreshed_callback_ =
        base::BindOnce(&AnboxAppListPrefs::OnAppListRefreshed,
  ...
  LOG(INFO) << "===== AnboxAppListPrefs::OnAppListRefreshed";
  ...
  if (!is_initialized_) {
    ...
    if (pai_starter) {
      pai_starter->AddOnStartCallback(
          base::BindOnce(&AnboxAppListPrefs::MaybeSetDefaultAppLoadingTimeout,
                         weak_ptr_factory_.GetWeakPtr()));
    } else {
...
void AnboxAppListPrefs::DetectDefaultAppAvailability() {
  LOG(INFO) << "===== AnboxAppListPrefs::DetectDefaultAppAvailability";
...
void AnboxAppListPrefs::MaybeSetDefaultAppLoadingTimeout() {  
  ...
  //         &AnboxAppListPrefs::DetectDefaultAppAvailability);
  //     break;
  //   }
  // }
}

void AnboxAppListPrefs::AddApp(const arc::mojom::AppInfo& app_info) {
...
void AnboxAppListPrefs::OnAppAddedDeprecated(arc::mojom::AppInfoPtr app) {
  AddApp(*app);
}

void AnboxAppListPrefs::InvalidateAppIcons(const std::string& app_id) {
...
void AnboxAppListPrefs::InvalidatePackageIcons(const std::string& package_name) {
  if (package_name == kFrameworkPackageName) {
    VLOG(1)
...
void AnboxAppListPrefs::ScheduleAppFolderDeletion(const std::string& app_id) {
...
void AnboxAppListPrefs::OnPackageAppListRefreshed(
    const std::string& package_name,
    std::vector<arc::mojom::AppInfoPtr> apps) {
  LOG(INFO) << "===== AnboxAppListPrefs::OnPackageAppListRefreshed";
...
void AnboxAppListPrefs::OnInstallShortcut(arc::mojom::ShortcutInfoPtr shortcut) {
...
void AnboxAppListPrefs::OnUninstallShortcut(const std::string& package_name,
                                          const std::string& intent_uri) {
...
std::unordered_set<std::string> AnboxAppListPrefs::GetAppsForPackage(
    const std::string& package_name) const {

  LOG(INFO) << "AnboxAppListPrefs::GetAppsForPackage-0 " << package_name;

  for (auto const &n: GetAppsAndShortcutsForPackage(package_name,
...
std::unordered_set<std::string> AnboxAppListPrefs::GetAppsAndShortcutsForPackage(
    const std::string& package_name,
...
void AnboxAppListPrefs::HandlePackageRemoved(const std::string& package_name) {
  DCHECK(IsArcAndroidEnabledForProfile(profile_));
...
void AnboxAppListPrefs::OnPackageRemoved(const std::string& package_name) {
...
void AnboxAppListPrefs::OnIcon(const std::string& app_id,
                             const ArcAppIconDescriptor& descriptor,
                             const std::vector<uint8_t>& icon_png_data) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  LOG(INFO) << "==== AnboxAppListPrefs::OnIcon " << app_id;
...
void AnboxAppListPrefs::OnIconResized(const std::string& app_id,
                                    const ArcAppIconDescriptor& descriptor,
                                    const std::vector<uint8_t>& icon_png_data) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  if (!icon_png_data.empty())
...
void AnboxAppListPrefs::DiscardResizeRequest(ResizeRequest* request) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void AnboxAppListPrefs::OnTaskCreated(int32_t task_id,
                                    const std::string& package_name,
                                    const std::string& activity,
                                    const base::Optional<std::string>& name,
                                    const base::Optional<std::string>& intent) {
  LOG(INFO) << "===== AnboxAppListPrefs::OnTaskCreated " << task_id << " " << name.value() << " " << package_name << " " << activity;

  HandleTaskCreated(name, package_name, activity);
  for (auto& observer : observer_list_) {
    // observer.OnTaskSetActive(-1);
...
void AnboxAppListPrefs::OnTaskDescriptionUpdated(
    int32_t task_id,
...
void AnboxAppListPrefs::OnTaskDestroyed(int32_t task_id) {
  for (auto& observer : observer_list_)
    observer.OnTaskDestroyed(task_id);
}

void AnboxAppListPrefs::OnTaskSetActive(int32_t task_id) {
  for (auto& observer : observer_list_)
    observer.OnTaskSetActive(task_id);
}

void AnboxAppListPrefs::OnNotificationsEnabledChanged(
    const std::string& package_name,
    bool enabled) {
  const base::DictionaryValue* apps =
      prefs_->GetDictionary(arc::prefs::kArcApps);
...
bool AnboxAppListPrefs::IsUnknownPackage(const std::string& package_name) const {
  if (GetPackage(package_name))
    return false;
...
bool AnboxAppListPrefs::IsDefaultPackage(const std::string& package_name) const {
...
void AnboxAppListPrefs::OnPackageAdded(
...
void AnboxAppListPrefs::OnPackageModified(
    arc::mojom::ArcPackageInfoPtr package_info) {
  DCHECK(IsArcAndroidEnabledForProfile(profile_));
  AddOrUpdatePackagePrefs(*package_info);
  for (auto& observer : observer_list_)
...
void AnboxAppListPrefs::OnPackageListRefreshed(
...
std::vector<std::string> AnboxAppListPrefs::GetPackagesFromPrefs() const {
  return GetPackagesFromPrefs(true /* check_arc_alive */, true /* installed */);
}

std::vector<std::string> AnboxAppListPrefs::GetPackagesFromPrefs(
...
base::Time AnboxAppListPrefs::GetInstallTime(const std::string& app_id) const {
  const base::DictionaryValue* app = nullptr;
  const base::DictionaryValue* apps =
      prefs_->GetDictionary(arc::prefs::kArcApps);
  if (!apps || !apps->GetDictionaryWithoutPathExpansion(app_id, &app))
...
void AnboxAppListPrefs::InstallIcon(const std::string& app_id,
  ...
  //     base::BindOnce(&AnboxAppListPrefs::OnIconInstalled,
  //                    weak_ptr_factory_.GetWeakPtr(), app_id, descriptor));
}
...
void AnboxAppListPrefs::OnInstallationStarted(
...
void AnboxAppListPrefs::OnInstallationFinished(
    arc::mojom::InstallationResultPtr result) {
  LOG(INFO) << "======== AnboxAppListPrefs::OnInstallationFinished";
...
void AnboxAppListPrefs::NotifyAppStatesChanged(const std::string& app_id) {
...
void AnboxAppListPrefs::onAppAdded(void *pItem){
  anbox::application::Database::Item *pAppItem = (anbox::application::Database::Item*)pItem;

  LOG(INFO) << "======== AnboxAppListPrefs::onAppAdded " << 
    pAppItem->name << " " << pAppItem->launch_intent.action << " " <<
    pAppItem->launch_intent.component;

  auto app_id = GetAppId(pAppItem->package, pAppItem->launch_intent.component);
  auto icon_dec = ArcAppIconDescriptor{48, ui::ScaleFactor::SCALE_FACTOR_100P};
  LOG(INFO) << "======== AnboxAppListPrefs::onAppAdded app_id " << app_id << " " << pAppItem->icon.size();

  auto result = InstallIconFromFileThread(
    GetIconPath(app_id, icon_dec),
    pAppItem->icon
  );

  OnIconInstalled(app_id, icon_dec, result);
  ...
  file_task_runner_->PostTask(FROM_HERE,
    base::BindOnce(
      ...
      weak_ptr_factory_.GetWeakPtr(), 
      std::string(pAppItem->name), std::string(pAppItem->package), std::string(pAppItem->launch_intent.component),
      std::string() /* intent_uri */, std::string() /* icon_resource_id */,
...
void AnboxAppListPrefs::onAppRemoved(void *pItem){
  anbox::application::Database::Item *pAppItem = (anbox::application::Database::Item*)pItem;  

  LOG(INFO) << "======== AnboxAppListPrefs::onAppRemoved " << pAppItem->name;
}

void AnboxAppListPrefs::OnTaskCreated(int task_id, const arc::AnboxSession::AppInfo &app_info){
  OnTaskCreated(task_id, app_info.package, app_info.component, app_info.name, base::nullopt);
}

// static
void AnboxAppListPrefs::AppInfo::SetIgnoreCompareInstallTimeForTesting(
    bool ignore) {
  ignore_compare_app_info_install_time = ignore;
}

AnboxAppListPrefs::AppInfo::AppInfo(const std::string& name,
                                  const std::string& package_name,
                                  const std::string& activity,
                                  const std::string& intent_uri,
                                  const std::string& icon_resource_id,
...
AnboxAppListPrefs::AppInfo::AppInfo(const AppInfo& other) = default;

// Need to add explicit destructor for chromium style checker error:
// Complex class/struct needs an explicit out-of-line destructor
AnboxAppListPrefs::AppInfo::~AppInfo() = default;

bool AnboxAppListPrefs::AppInfo::operator==(const AppInfo& other) const {
...
AnboxAppListPrefs::PackageInfo::PackageInfo(
    const std::string& package_name,
...
// Need to add explicit destructor for chromium style checker error:
// Complex class/struct needs an explicit out-of-line destructor
AnboxAppListPrefs::PackageInfo::~PackageInfo() = default;
