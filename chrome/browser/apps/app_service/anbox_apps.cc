...
void AddPreferredApp(const std::string& app_id,
  ...
  std::unique_ptr<AnboxAppListPrefs::AppInfo> app_info = prefs->GetApp(app_id);
...
bool ShouldShow(const AnboxAppListPrefs::AppInfo& app_info) {
...
// static
AnboxApps* AnboxApps::Get(Profile* profile) {  
  return AnboxAppsFactory::GetForProfile(profile);
}

// static
AnboxApps* AnboxApps::CreateForTesting(Profile* profile,
                                   apps::AppServiceProxy* proxy) {
  return new AnboxApps(profile, proxy);
}

AnboxApps::AnboxApps(Profile* profile) : AnboxApps(profile, nullptr) {  }

AnboxApps::AnboxApps(Profile* profile, apps::AppServiceProxy* proxy)
    : profile_(profile)/*, arc_icon_once_loader_(profile)*/ {
  LOG(INFO) << "======== AnboxApps::AnboxApps 1";
  ...
  LOG(INFO) << "======== AnboxApps::AnboxApps 3 " << AnboxAppListPrefs::Get(profile_);
  ...
  LOG(INFO) << "======== AnboxApps::AnboxApps 4";
  ...
  LOG(INFO) << "======== AnboxApps::AnboxApps 5";                               
}

AnboxApps::~AnboxApps() = default;

void AnboxApps::SetTaskActive(int task_id){
  
}

void AnboxApps::Shutdown() {
  // Disconnect the observee-observer connections that we made during the
  // constructor.
  //
  // This isn't entirely correct. The object returned by
  // AnboxAppListPrefs::Get(some_profile) can vary over the lifetime of that
  // profile. If it changed, we'll try to disconnect from different
  // AnboxAppListPrefs-related objects than the ones we connected to, at the time
  ...
  // OnArcAppListPrefsWillBeDestroyed method to AnboxAppListPrefs::Observer, and
  // in this class's implementation of that method, disconnect. Furthermore,
  // when the new AnboxAppListPrefs object is created, we'll have to somehow be
...
void AnboxApps::Connect(
    mojo::PendingRemote<apps::mojom::Subscriber> subscriber_remote,
    apps::mojom::ConnectOptionsPtr opts) {
      
  LOG(INFO) << "========== AnboxApps::Connect";
...
void AnboxApps::LoadIconFromVM(const std::string app_id,
                                  apps::mojom::IconCompression icon_compression,
                                  int32_t size_hint_in_dip,
...
void AnboxApps::LoadIcon(const std::string& app_id,
  ...
  if (app_id.length() > 0){
    ...
    auto p1 = icon_path.AppendASCII(/*arc::prefs::kArcApps*/ "arc.apps")
            .AppendASCII(app_id)
            .AppendASCII(
              ArcAppIconDescriptor{48, ui::ScaleFactor::SCALE_FACTOR_100P}.GetName()
            );

    // LOG(INFO) << "===== AnboxApps::LoadIcon " << app_id << " " << icon_path;    

    LoadIconFromFileWithFallback(icon_compression, size_hint_in_dip, 
      ...
      base::BindOnce(&AnboxApps::LoadIconFromVM,
  ...
  if (app_id == arc::kPlayStoreAppId) {
    ...
    const AnboxAppListPrefs* arc_prefs = AnboxAppListPrefs::Get(profile_);
    DCHECK(arc_prefs);
...
void AnboxApps::Launch(const std::string& app_id,
                     int32_t event_flags,
                     apps::mojom::LaunchSource launch_source,
                     int64_t display_id) {
  ...
  auto anbox_app_list_prefs = AnboxAppListPrefs::Get(profile_);
  if (anbox_app_list_prefs){
    auto app_info = anbox_app_list_prefs->GetApp(app_id);    
    ...
    if (anbox_session_manager){                
      anbox_session_manager->GetAnboxSession()->LaunchApp(app_info->name, app_info->package_name, app_info->activity);      
    }
  }    
...
void AnboxApps::LaunchAppWithFiles(const std::string& app_id,
...
void AnboxApps::LaunchAppWithIntent(const std::string& app_id,
                                  apps::mojom::IntentPtr intent,
                                  apps::mojom::LaunchSource launch_source,
                                  int64_t display_id) {
  LOG(INFO) << "====== AnboxApps::LaunchAppWithIntent " << app_id;
  ...
  const std::unique_ptr<AnboxAppListPrefs::AppInfo> app_info =
      prefs->GetApp(app_id);
  if (!app_info) {
...
void AnboxApps::SetPermission(const std::string& app_id,
                            apps::mojom::PermissionPtr permission) {
  LOG(INFO) << "====== AnboxApps::SetPermission " << app_id;
...
void AnboxApps::PauseApp(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::PauseApp " << app_id;                        

  paused_apps_.MaybeAddApp(app_id);
  constexpr bool kPaused = true;
  Publish(paused_apps_.GetAppWithPauseStatus(apps::mojom::AppType::kAnbox, app_id,
                                             kPaused));
...
void AnboxApps::UnpauseApps(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::UnpauseApps " << app_id;                        
...
void AnboxApps::GetMenuModel(const std::string& app_id,
...
void AnboxApps::OpenNativeSettings(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::OpenNativeSettings " << app_id;                        
...
void AnboxApps::OnPreferredAppSet(
    const std::string& app_id,
    apps::mojom::IntentFilterPtr intent_filter,
    apps::mojom::IntentPtr intent,
    apps::mojom::ReplacedAppPreferencesPtr replaced_app_preferences) {
  LOG(INFO) << "====== AnboxApps::OnPreferredAppSet " << app_id;                        
...
void AnboxApps::OnAppRegistered(const std::string& app_id,
                              const AnboxAppListPrefs::AppInfo& app_info) {
  LOG(INFO) << "====== AnboxApps::OnAppRegistered " << app_id << " " << 
...
void AnboxApps::OnAppStatesChanged(const std::string& app_id,
                                 const AnboxAppListPrefs::AppInfo& app_info) {
  LOG(INFO) << "====== AnboxApps::OnAppStatesChanged " << app_id;                        
...
void AnboxApps::OnAppRemoved(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::OnAppRemoved " << app_id;                        
  ...
  app->app_type = apps::mojom::AppType::kAnbox;
  app->app_id = app_id;
  ...
  app_service->RemovePreferredApp(apps::mojom::AppType::kAnbox, app_id);
}

void AnboxApps::OnAppIconUpdated(const std::string& app_id,
                               const ArcAppIconDescriptor& descriptor) {
  LOG(INFO) << "====== AnboxApps::OnAppIconUpdated " << app_id;                        
...
void AnboxApps::OnAppNameUpdated(const std::string& app_id,
                               const std::string& name) {
  LOG(INFO) << "====== AnboxApps::OnAppNameUpdated " << app_id;                        
...
void AnboxApps::OnAppLastLaunchTimeUpdated(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::OnAppLastLaunchTimeUpdated " << app_id;                        
...
void AnboxApps::OnPackageInstalled(
    const arc::mojom::ArcPackageInfo& package_info) {
  LOG(INFO) << "====== AnboxApps::OnPackageInstalled ";

  ConvertAndPublishPackageApps(package_info);
}

void AnboxApps::OnPackageModified(
    const arc::mojom::ArcPackageInfo& package_info) {
  LOG(INFO) << "====== AnboxApps::OnPackageModified ";

  static constexpr bool update_icon = false;
  ConvertAndPublishPackageApps(package_info, update_icon);
}

void AnboxApps::OnPackageListInitialRefreshed() {
  LOG(INFO) << "====== AnboxApps::OnPackageListInitialRefreshed ";
...
void AnboxApps::OnTaskCreated(int task_id,
                            const std::string& package_name,
                            const std::string& activity,
                            const std::string& intent) {
  LOG(INFO) << "====== AnboxApps::OnTaskCreated " << task_id << " " << package_name << " " << activity;  

  const std::string app_id = AnboxAppListPrefs::GetAppId(package_name, activity);
  app_id_to_task_ids_[app_id].insert(task_id);
  task_id_to_app_id_[task_id] = app_id;
}

void AnboxApps::OnTaskDestroyed(int task_id) {
  LOG(INFO) << "====== AnboxApps::OnTaskDestroyed " << task_id;
...
void AnboxApps::OnIntentFiltersUpdated(
    const base::Optional<std::string>& package_name) {
  LOG(INFO) << "====== AnboxApps::OnIntentFiltersUpdated";
...
void AnboxApps::OnPreferredAppsChanged() {
  LOG(INFO) << "====== AnboxApps::OnPreferredAppsChanged";
  ...
  for (auto& added_preferred_app : added_preferred_apps) {
    ...
    app_service->AddPreferredApp(apps::mojom::AppType::kAnbox, app_id,
  ...
  for (auto& deleted_preferred_app : deleted_preferred_apps) {
    ...
    app_service->RemovePreferredAppForFilter(
        apps::mojom::AppType::kAnbox, app_id,
        ConvertArcIntentFilter(deleted_preferred_app));
  }
}

void AnboxApps::LoadPlayStoreIcon(apps::mojom::IconCompression icon_compression,
                                int32_t size_hint_in_dip,
                                IconEffects icon_effects,
                                LoadIconCallback callback) {
  LOG(INFO) << "====== AnboxApps::LoadPlayStoreIcon";
...
apps::mojom::AppPtr AnboxApps::Convert(AnboxAppListPrefs* prefs,
                                     const std::string& app_id,
                                     const AnboxAppListPrefs::AppInfo& app_info,
                                     bool update_icon) {
  apps::mojom::AppPtr app = apps::mojom::App::New();  
  ...
  std::unique_ptr<AnboxAppListPrefs::PackageInfo> package =
      prefs->GetPackage(app_info.package_name);
  if (package) {
    UpdateAppPermissions(package->permissions, &app->permissions);
  }else{
    LOG(INFO) << "==== AnboxApps::Convert not found package_name " << app_info.package_name;
  }
...
void AnboxApps::ConvertAndPublishPackageApps(
    const arc::mojom::ArcPackageInfo& package_info,
    bool update_icon) {

  LOG(INFO) << "=== AnboxApps::ConvertAndPublishPackageApps " << package_info.permissions.has_value();
...
void AnboxApps::SetIconEffect(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::SetIconEffect " << app_id;
...
void AnboxApps::CloseTasks(const std::string& app_id) {
  LOG(INFO) << "====== AnboxApps::CloseTasks " << app_id;
