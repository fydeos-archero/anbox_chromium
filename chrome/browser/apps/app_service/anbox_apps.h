...
#ifndef CHROME_BROWSER_APPS_APP_SERVICE_ANBOX_APPS_H_
#define CHROME_BROWSER_APPS_APP_SERVICE_ANBOX_APPS_H_
...
class AnboxApps : public KeyedService,
                public apps::mojom::Publisher,
                public AnboxAppListPrefs::Observer,
                public arc::ArcIntentHelperObserver {
 public:
  static AnboxApps* Get(Profile* profile);

  static AnboxApps* CreateForTesting(Profile* profile,
                                   apps::AppServiceProxy* proxy);

  explicit AnboxApps(Profile* profile);

  ~AnboxApps() override;

 public:
  static void SetTaskActive(int task_id);
  
 private:
  using AppIdToTaskIds = std::map<std::string, std::set<int>>;
  using TaskIdToAppId = std::map<int, std::string>;

  AnboxApps(Profile* profile, apps::AppServiceProxy* proxy);
  ...
  apps::mojom::AppPtr Convert(AnboxAppListPrefs* prefs,
                              const std::string& app_id,
  ...
  base::WeakPtrFactory<AnboxApps> weak_ptr_factory_{this};

  DISALLOW_COPY_AND_ASSIGN(AnboxApps);
};

}  // namespace apps

#endif  // CHROME_BROWSER_APPS_APP_SERVICE_ANBOX_APPS_H_
